...
class BotBuilder {
  ...
  async _post_commands(): Promise<void> {
    ...
    if (client.user) {
      ...
      const commands = SlashLoader.commands.map((command) => command.data);
      const GLOBAL: boolean = process.env.GLOBAL
        ? JSON.parse(process.env.GLOBAL)
        : false;
